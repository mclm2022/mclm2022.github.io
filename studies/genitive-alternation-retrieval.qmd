...
```{r}
#| label: read
brown_fnames <- get_fnames(file.path(corpus_folder, "brown")) %>% 
  keep_re("/c[a-z][0-9]{2}")
print(brown_fnames, n = 10)
```
...
examples[["_corpora/brown/ca01_1"]]
```

Then you could write, in an empty line of your report, a numbered example with R inline code: ``(@label) `r knitr::inline_expr('examples[["_corpora/brown/ca01_1"]]')` ``. Then you can easily cite the example with `(@label)`, which produces (@label).

(@label) `r examples[["_corpora/brown/ca01_1"]]`
